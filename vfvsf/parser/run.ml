...
let rec parseTree tree = 
    ...
    let handleStreamV streamN = 
        ...
        with Not_found -> (Printf.fprintf stderr "Stream %d is empty\nLine number:\t%d\nUnable to complete execution" streamN !lineN); flush stderr; (exit 1)
    in
    ...
    let error x1 = 
        (Printf.fprintf stderr "Error: Invalid syntax \"%s\"\nLine number:\t%d\nUnable to complete execution" x1 !lineN); flush stderr; (exit 2)
    in
    ...
    match tree with 
        ...
        | Node2( "assG", Name( x1, line1 ), Leaf( x2, line2), line3)    
                                                  -> lineN := line3;  (handleAssG x1 x2)
...
let getInStreams = 
    let rec collect_lists n k =
         match n with
          | 0 -> []
          | x -> ref (List.map int_of_string (Str.split (Str.regexp " ") (read_line ()))) :: collect_lists (x - 1) k in
    let helper a b =
        streamC := b;
        streamLen := a;
        List.rev (collect_lists b a) in
